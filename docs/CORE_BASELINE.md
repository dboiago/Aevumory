...
* **Default Interaction Simplicity:** The normal task interaction is `view → perform → complete`. Discipline mechanics must resolve automatically around this interaction and must not introduce additional user workflow unless explicitly required by the mechanic. Foothold and Deductive Pruning are intentional exceptions.
* **Mastery Removes Boundaries:** Mastery should expand the contexts in which a Discipline can meaningfully operate. It should remove artificial restrictions or enable qualitatively broader application of the underlying capability, rather than merely increasing numerical rewards.
